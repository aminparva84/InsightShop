...
if [ -f scripts/test_hf_api_simple.py ]; then
    sed -i "s/YOUR_ACTUAL_TOKEN_HERE/YOUR_TOKEN_HERE/g" scripts/test_hf_api_simple.py
...
if [ -f scripts/test_inference_client.py ]; then
    sed -i "s/YOUR_ACTUAL_TOKEN_HERE/YOUR_TOKEN_HERE/g" scripts/test_inference_client.py
